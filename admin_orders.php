    ...
    if ($action === 'create_invoices_for_order_date') {
        ...
        try {
            ...
            foreach ($groups as $group) {
                ...
                // Send email with PDF if user has email address
                try {
                    $emailStmt = $pdo->prepare("SELECT email FROM users WHERE id = ? LIMIT 1");
                    $emailStmt->execute([(int)$group['user_id']]);
                    $userEmail = trim((string)($emailStmt->fetchColumn() ?? ''));

                    if ($userEmail !== '') {
                        $pdfBytes = generateInvoicePdf($pdo, $publicToken);
                        $subject  = 'Deine Rechnung ' . $invoiceNumber;
                        $html     = '
                            <p>Hallo ' . htmlspecialchars($recipientName, ENT_QUOTES, 'UTF-8') . ',</p>
                            <p>deine Rechnung <strong>' . htmlspecialchars($invoiceNumber, ENT_QUOTES, 'UTF-8') . '</strong> wurde erstellt.</p>
                            <p>Im Anhang findest du die Rechnung als PDF.</p>
                            <p>Mit freundlichen Grüßen<br>Das Einkauf-Team</p>
                        ';

                        sendMail($userEmail, $recipientName, $subject, $html, $pdfBytes !== null ? [
                    ...
                    // Mail failure must not abort the invoice creation
                }
            }
